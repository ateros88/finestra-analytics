...
if not st.session_state["user"]:
    st.markdown("<h1 style='text-align: center;'>Finestra Analytics</h1>", unsafe_allow_html=True)
    st.markdown("<h3 style='text-align: center; color: gray;'>Betatest</h3>", unsafe_allow_html=True)
    st.write("")
    ...
    with col2:
        ...
        with tab_signup:
            ...
            new_password = st.text_input("Välj lösenord", type="password", key="signup_password")
...
TIER_FEATURES = {
    ...
    'advance': ['top4_dashboard', 'full_dashboard', 'sector_selection', 'sector_rotation', 'deep_analysis'],
    'master': ['top4_dashboard', 'full_dashboard', 'sector_selection', 'sector_rotation', 'deep_analysis', 'premium_blog']
}
...
sektor_namn_sv = {
    'Technology': 'Teknologi', 'Financial Services': 'Finans', 'Healthcare': 'Hälsovård', 
    'Consumer Cyclical': 'Konsument (Sällanköp)', 'Consumer Defensive': 'Konsument (Bas)', 
    'Industrials': 'Industri', 'Energy': 'Energi', 'Utilities': 'Kraftförsörjning', 
    'Basic Materials': 'Material', 'Real Estate': 'Fastigheter', 'Communication Services': 'Kommunikation'
}

# --- 3. UI Layout ---
# Toppsektion med titel och diskret inloggningsinfo i högerkanten
header_col1, header_col2 = st.columns([0.6, 0.4])

with header_col1:
    st.markdown("<h1 style='color: black; margin-bottom: 0;'>Finestra Analytics</h1>", unsafe_allow_html=True)

with header_col2:
    user_email = st.session_state['user'].email
    # Samla e-post och knapp snyggt i en rad med HTML/CSS
    st.markdown(f"""
    ...
    # Eftersom Streamlit-knappar behöver ligga i python-flödet lägger vi den i en liten underkolumn här om vi vill, 
    # eller så sätter vi den direkt under. Men enklast för att få den helt till höger:
...
tabs = st.tabs(["DASHBOARD", "MARKET RESEARCH", "PRICING", "FAQ", "KONTO", "INSTÄLLNINGAR"])
...
with tabs[0]:
    ...
    if not df.empty:
        df['potential'] = pd.to_numeric(df['potential'], errors='coerce').fillna(0)
        df['nuvarande'] = pd.to_numeric(df['nuvarande'], errors='coerce').fillna(0)
        df['target'] = pd.to_numeric(df['target'], errors='coerce').fillna(0)
        df['antal_koprek'] = pd.to_numeric(df['antal_koprek'], errors='coerce').fillna(0)
        ...
        df['Kurs'] = "$" + df['nuvarande'].round(2).astype(str)
        df['Riktkurs'] = "$" + df['target'].round(2).astype(str)
        df['Potential (%)'] = df['potential'].round(1).astype(str) + " %"
        
        display_df = df.rename(columns={
            'ticker': 'Ticker', 'name': 'Namn', 'sektor_sv': 'Sektor', 'antal_koprek': 'Köprekar'
        })

        if has_access(st.session_state['user_tier'], 'sector_selection'):
            unika_sektorer = sorted([s for s in df['sektor_sv'].unique() if s and s != "N/A"])
            ...
            if vald_sektor == "Visa Alla (Topp 4)":
                ...
                visnings_df = display_df[display_df['Sektor'] == vald_sektor].sort_values(by='Finestra Score', ascending=False)
        else:
            st.info("Visar Topp 4 (Insight). Uppgradera till Advance för sektorval!")
            visnings_df = display_df.sort_values(by='Finestra Score', ascending=False).head(4)
        
        st.dataframe(
            visnings_df[['Ticker', 'Namn', 'Finestra Score', 'Kurs', 'Riktkurs', 'Potential (%)', 'Köprekar']], 
            width=1000, 
            hide_index=True
        )
...
with tabs[1]:
    ...
    sektor_namn = {
        'XLK': 'Teknologi', 'XLF': 'Finans', 'XLV': 'Hälsovård', 'XLY': 'Konsument (Sällanköp)',
        'XLP': 'Konsument (Bas)', 'XLI': 'Industri', 'XLE': 'Energi', 'XLU': 'Kraftförsörjning',
        'XLB': 'Material', 'XLRE': 'Fastigheter'
    }
    ...
    if has_access(st.session_state['user_tier'], 'sector_rotation'):
        try:
            ...
            if not df_sektor.empty:
                ...
                df_sektor['förändring'] = df_sektor.groupby('ticker')['pris'].transform(lambda x: (x / x.iloc[0] - 1) * 100)
                df_sektor['sektor_namn'] = df_sektor['ticker'].map(sektor_namn)
                
                fig = px.line(
                    df_sektor, x='datum', y='förändring', color='sektor_namn',
                    labels={'förändring': 'Avkastning (%)', 'datum': 'Datum', 'sektor_namn': 'Sektor'}
                )
                
                fig.update_layout(hovermode="x unified", yaxis_ticksuffix=" %", xaxis_title="", yaxis_title="Avkastning (%)")
                fig.update_xaxes(tickformat="%Y-%m-%d") 
                
                st.plotly_chart(fig, width='stretch')
                st.caption("Grafen visar procentuell utveckling sedan första mätningen. Datan är baserad på amerikanska SPDR Sector ETFs.")
                ...
                df_senaste = df_senaste[['sektor_namn', 'förändring']].sort_values(by='förändring', ascending=False)
                df_senaste['förändring'] = df_senaste['förändring'].round(2).astype(str) + " %"
                df_senaste = df_senaste.rename(columns={'sektor_namn': 'Sektor', 'förändring': 'Utveckling'})
...
# --- PRICING ---
with tabs[2]:
    st.markdown("<h2 style='text-align: center;'>Välj din nivå</h2>", unsafe_allow_html=True)
    st.write("")
...
# --- FAQ ---
with tabs[3]:
    st.markdown("<h2 style='text-align: center;'>Vanliga frågor och svar</h2>", unsafe_allow_html=True)
    st.write("")
    
    with st.expander("Vad är Finestra Analytics?"):
        st.write("Finestra Analytics är en digital plattform som kombinerar datadrivna aktieanalyser, sektorrotation och djupgående marknadsutbildning för att hjälpa dig göra bättre investeringsbeslut.")
    
    with st.expander("Vad är Finestra score?"):
        st.write("Finestra score är vårt egna sammanvägda betyg som rankar aktier baserat på flera olika kvantitativa parametrar. Det hjälper dig att snabbt sålla ut vilka bolag som presterar starkast enligt vår modell.")
    ...
    with st.expander("Hur ofta uppdateras innehållet?"):
        st.write("Topplistan och marknadsdata uppdateras löpande. Vårt utbildningsmaterial och våra djupanalyser/rapporter uppdateras regelbundet för att säkerställa högsta kvalitet.")
    
    with st.expander("Kan jag säga upp min prenumeration när som helst?"):
        st.write("Ja, absolut. Det är ingen bindningstid, du avslutar enkelt din prenumeration direkt via ditt konto när du vill.")
    
    with st.expander("Ger ni personliga finansiella råd?"):
        st.write("Nej. Finestra Analytics tillhandahåller analysverktyg, marknadsdata och utbildning. Alla investeringar sker på eget ansvar.")
...
with tabs[4]: 
    ...
    st.selectbox(
        ...
        index=['insight', 'advance', 'master'].index(st.session_state.get('user_tier', 'insight'))
    )
